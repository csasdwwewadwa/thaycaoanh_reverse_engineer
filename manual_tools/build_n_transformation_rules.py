...
from rules.n_transformation_stars import n_transformation_template_key
        ...
        for column in COLUMNS:
            for raw_star_id in palace.get(column, ()):
                ...
                if star_id in STAR_IDS:
                    ...
                    if star_id in found:
                        raise ValueError(f"Repeated N transformation ID: {star_id}")
                    found.add(star_id)
    if found != STAR_IDS:
        raise ValueError(f"Unexpected N transformation IDs: {sorted(found)}")
    return template
...
def run(args):
    ...
    with args.dataset.open("r", encoding="utf-8") as source:
        for line_number, line in enumerate(source, 1):
            ...
            key = ",".join(
                str(value)
                for value in n_transformation_template_key(
                    input_data["day"],
                    input_data["month"],
                    input_data["year"],
                    input_data["hour"],
            ...
            if previous != template:
                raise ValueError(f"Conflicting N transformation template at line {line_number}: key {key}")
            rows += 1
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build N transformation physical-slot rules.")
    parser.add_argument("dataset", type=Path, nargs="?", default=Path("metadata copy.jsonl"))
    parser.add_argument("--output", type=Path, default=Path("rules/generated/n_transformation_stars.json"))
