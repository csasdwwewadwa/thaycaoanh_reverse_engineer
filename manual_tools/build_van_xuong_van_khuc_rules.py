...
from rules.van_xuong_van_khuc_stars import van_xuong_van_khuc_template_key


FAMILIES = ({45, 194}, {79, 191})
...
def output_template(item):
    ...
    if any(count != 1 for count in found):
        raise ValueError(f"Unexpected Van Xuong/Van Khuc family counts: {found}")
    return template
...
def run(args):
    ...
    with args.dataset.open("r", encoding="utf-8") as source:
        for line_number, line in enumerate(source, 1):
            ...
            key = ",".join(
                str(value)
                for value in van_xuong_van_khuc_template_key(
            ...
            if previous != template:
                raise ValueError(f"Conflicting Van Xuong/Van Khuc template at line {line_number}: key {key}")
            rows += 1
    ...
    artifact = {
        ...
        "rows_validated": rows,
        "template_key": ["rolled_lunar_month", "lunar_leap", "hour_branch"],
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build Van Xuong and Van Khuc physical-slot rules.")
    parser.add_argument("dataset", type=Path, nargs="?", default=Path("metadata copy.jsonl"))
    parser.add_argument("--output", type=Path, default=Path("rules/generated/van_xuong_van_khuc_stars.json"))
