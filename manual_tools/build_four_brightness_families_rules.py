...
from rules.four_brightness_families_stars import four_brightness_families_template_key


FAMILIES = ({18, 168}, {39, 146}, {48, 154}, {85, 165})
STAR_IDS = set().union(*FAMILIES)
...
def output_template(item):
    ...
    if any(count != 1 for count in found):
        raise ValueError(f"Unexpected four brightness family counts: {found}")
    return template
...
def run(args):
    ...
    with args.dataset.open("r", encoding="utf-8") as source:
        for line_number, line in enumerate(source, 1):
            ...
            key = ",".join(
                str(value)
                for value in four_brightness_families_template_key(
                    input_data["day"],
                    input_data["month"],
                    input_data["year"],
                    input_data["hour"],
            ...
            if previous != template:
                raise ValueError(
                    f"Conflicting four brightness families template at line {line_number}: key {key}"
                )
            rows += 1
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build four brightness-family physical-slot rules.")
    parser.add_argument("dataset", type=Path, nargs="?", default=Path("metadata copy.jsonl"))
    parser.add_argument(
        "--output", type=Path, default=Path("rules/generated/four_brightness_families_stars.json")
    )
