...
from rules.hao_brightness_stars import hao_brightness_template_key


FAMILIES = ({12, 138}, {126, 140})
...
def output_template(item):
    ...
    if any(count != 1 for count in found):
        raise ValueError(f"Unexpected Hao brightness family counts: {found}")
    return template
...
def run(args):
    ...
    with args.dataset.open("r", encoding="utf-8") as source:
        for line_number, line in enumerate(source, 1):
            ...
            key = ",".join(
                str(value)
                for value in hao_brightness_template_key(
                    input_data["day"],
                    input_data["month"],
                    input_data["year"],
                    input_data["hour"],
            ...
            if previous != template:
                raise ValueError(f"Conflicting Hao brightness template at line {line_number}: key {key}")
            rows += 1
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build Hao brightness physical-slot rules.")
    parser.add_argument("dataset", type=Path, nargs="?", default=Path("metadata copy.jsonl"))
    parser.add_argument("--output", type=Path, default=Path("rules/generated/hao_brightness_stars.json"))
