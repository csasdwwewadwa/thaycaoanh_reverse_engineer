...
from rules.thien_tru_duong_phu_stars import thien_tru_duong_phu_template_key
...
def output_template(item):
    ...
    if found != len(STAR_IDS):
        raise ValueError(f"Expected {len(STAR_IDS)} Thien Tru/Duong Phu stars, found {found}")
    return template
...
def run(args):
    ...
    with args.dataset.open("r", encoding="utf-8") as source:
        for line_number, line in enumerate(source, 1):
            ...
            key = str(
                thien_tru_duong_phu_template_key(
                    input_data["day"],
                    input_data["month"],
                    input_data["year"],
                    input_data["hour"],
            ...
            if previous != template:
                raise ValueError(f"Conflicting Thien Tru/Duong Phu template at line {line_number}: key {key}")
            rows += 1
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build Thien Tru and Duong Phu physical-slot rules.")
    parser.add_argument("dataset", type=Path, nargs="?", default=Path("metadata copy.jsonl"))
    parser.add_argument("--output", type=Path, default=Path("rules/generated/thien_tru_duong_phu_stars.json"))
