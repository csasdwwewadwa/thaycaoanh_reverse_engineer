...
from rules.tang_mon_brightness_stars import tang_mon_brightness_template_key
    ...
    for palace_index, palace in enumerate(item["output_chart"]["palaces"]):
        for raw_star_id in palace.get("right_stars", ()):
            ...
            if star_id in STAR_IDS:
                ...
                found += 1
    if found != 1:
        raise ValueError(f"Unexpected Tang Mon brightness state count: {found}")
    return template
...
def run(args):
    ...
    with args.dataset.open("r", encoding="utf-8") as source:
        for line_number, line in enumerate(source, 1):
            ...
            key = ",".join(
                str(value)
                for value in tang_mon_brightness_template_key(
                    input_data["day"],
                    input_data["month"],
                    input_data["year"],
                    input_data["hour"],
            ...
            if previous != template:
                raise ValueError(
                    f"Conflicting Tang Mon brightness template at line {line_number}: key {key}"
                )
            rows += 1
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build Tang Mon brightness physical-slot rules.")
    parser.add_argument("dataset", type=Path, nargs="?", default=Path("metadata copy.jsonl"))
    parser.add_argument(
        "--output", type=Path, default=Path("rules/generated/tang_mon_brightness_stars.json")
    )
