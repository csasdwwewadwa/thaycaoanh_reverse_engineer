...
from rules.thien_ma_stars import thien_ma_template_key
...
def output_template(item):
    ...
    if found != 1:
        raise ValueError(f"Expected one displayed Thien Ma state, found {found}")
    return template
...
def run(args):
    ...
    with args.dataset.open("r", encoding="utf-8") as source:
        for line_number, line in enumerate(source, 1):
            ...
            key = ",".join(
                str(value)
                for value in thien_ma_template_key(
            ...
            if previous != template:
                raise ValueError(f"Conflicting Thien Ma template at line {line_number}: key {key}")
            rows += 1
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build Thien Ma physical-slot rules.")
    parser.add_argument("dataset", type=Path, nargs="?", default=Path("metadata copy.jsonl"))
    parser.add_argument("--output", type=Path, default=Path("rules/generated/thien_ma_stars.json"))
