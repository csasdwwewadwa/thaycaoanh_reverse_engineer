...
from rules.thai_tue_series_stars import thai_tue_series_template_key


STAR_IDS = {19, 41, 49, 51, 64, 73, 74, 84, 86, 99, 107, 116}
...
def output_template(item):
    ...
    if found != len(STAR_IDS):
        raise ValueError(f"Expected {len(STAR_IDS)} Thai Tue series stars, found {found}")
    return template
...
def run(args):
    ...
    with args.dataset.open("r", encoding="utf-8") as source:
        for line_number, line in enumerate(source, 1):
            ...
            key = ",".join(
                str(value)
                for value in thai_tue_series_template_key(
                    input_data["day"],
                    input_data["month"],
                    input_data["year"],
                    input_data["hour"],
            ...
            if previous != template:
                raise ValueError(f"Conflicting Thai Tue series template at line {line_number}: key {key}")
            rows += 1
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build Thai Tue series physical-slot rules.")
    parser.add_argument("dataset", type=Path, nargs="?", default=Path("metadata copy.jsonl"))
    parser.add_argument("--output", type=Path, default=Path("rules/generated/thai_tue_series_stars.json"))
