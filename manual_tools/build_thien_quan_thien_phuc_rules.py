...
from rules.thien_quan_thien_phuc_stars import thien_quan_thien_phuc_template_key
...
def output_template(item):
    ...
    if found != len(STAR_IDS):
        raise ValueError(f"Expected {len(STAR_IDS)} Thien Quan/Thien Phuc stars, found {found}")
    return template
...
def run(args):
    ...
    with args.dataset.open("r", encoding="utf-8") as source:
        for line_number, line in enumerate(source, 1):
            ...
            key = str(
                thien_quan_thien_phuc_template_key(
                    input_data["day"],
                    input_data["month"],
                    input_data["year"],
                    input_data["hour"],
            ...
            if previous != template:
                raise ValueError(f"Conflicting Thien Quan/Thien Phuc template at line {line_number}: key {key}")
            rows += 1
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build Thien Quan and Thien Phuc physical-slot rules.")
    parser.add_argument("dataset", type=Path, nargs="?", default=Path("metadata copy.jsonl"))
    parser.add_argument("--output", type=Path, default=Path("rules/generated/thien_quan_thien_phuc_stars.json"))
