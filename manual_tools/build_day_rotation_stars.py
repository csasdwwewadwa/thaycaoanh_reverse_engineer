...
from rules.day_rotation_stars import day_rotation_template_key
    ...
    for palace_index, palace in enumerate(item["output_chart"]["palaces"]):
        for raw_star_id in palace.get("left_stars", ()):
            ...
            if star_id in TARGET_IDS:
                if star_id in found:
                    raise ValueError(f"Duplicate day-rotation star {star_id}")
                found.add(star_id)
                template[palace_index].append(star_id)
    if found != TARGET_IDS:
        raise ValueError(f"Unexpected day-rotation star set: {sorted(found)}")
    return template
...
def run(args):
    ...
    with args.dataset.open("r", encoding="utf-8") as source:
        for line_number, line in enumerate(source, 1):
            ...
            key = ",".join(
                str(value)
                for value in day_rotation_template_key(
            ...
            if previous != template:
                raise ValueError(f"Conflicting day-rotation template at line {line_number}: key {key}")
            rows += 1
    ...
    artifact = {
        ...
        "rows_validated": rows,
        "template_key": ["rolled_lunar_day", "rolled_lunar_month", "lunar_leap", "hour_branch"],
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build day-driven physical rotation star rules.")
    parser.add_argument("dataset", type=Path, nargs="?", default=Path("metadata copy.jsonl"))
    parser.add_argument("--output", type=Path, default=Path("rules/generated/day_rotation_stars.json"))
